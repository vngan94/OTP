...
function validate() {
    ...
    console.log(code)
    if (code == '') {
        $('#staffcode').addClass('warning-border'); $('#warning_text').text("Mã nhân viên trống!!"); return false
    } else {
    ...
    if (new Date(date).getTime() >= new Date().getTime() - 86400000 * 365 * 18) {
        $('#datepicker').addClass('warning-border'); $('#warning_text').text("Nhân viên phải lớn hơn 18 tuổi!!"); return false
    } else {
        $('#datepicker').removeClass('warning-border');
        $('#warning_text').text("")
    }
    if (!regex_username.test(username)) {
        $('#staffusername').addClass('warning-border'); $('#warning_text').text("Tên đăng nhập không hợp lệ!!"); return false
    } else {
