...
route.get('/fermeh/admin/orders', CatchAsync(async (req, res) => {
    if (req.cookies.role == '' || typeof (req.cookies.role) == 'undefined' || req.cookies.tknv == ''
        ...
        let pool = await conn
        await pool.request().query(`select MAHD,HUY,MAKH,NGAYLAP,TONGTIEN,TEN,PHUONGTHUC,TENKH=(select TENKH 
...
route.post('/fermeh/admin/orders/status-filter', CatchAsync(async (req, res) => {
    if (req.cookies.role == '' || typeof (req.cookies.role) == 'undefined' || req.cookies.tknv == ''
        || typeof (req.cookies.tknv) == 'undefined' || req.cookies.tentk == '' || typeof (req.cookies.tentk) == 'undefined') {
        res.redirect('http://localhost:3000/fermeh/admin/login')
    }
        ...
        var sql
        if (req.body.data == '1') {
            sql = `select MAHD,HUY,MAKH,NGAYLAP,TONGTIEN,TEN,PHUONGTHUC,TENKH=(select TENKH 
                from KHACHHANG where KHACHHANG.MAKH=HOADON.MAKH) from HOADON where HUY=1`
        } else if (req.body.data == '0') {
            sql = `select MAHD,HUY,MAKH,NGAYLAP,TONGTIEN,TEN,PHUONGTHUC,TENKH=(select TENKH 
                from KHACHHANG where KHACHHANG.MAKH=HOADON.MAKH) from HOADON where HUY=0`
        }
        else if (req.body.data == 'all') {
            sql = `select MAHD,HUY,MAKH,NGAYLAP,TONGTIEN,TEN,PHUONGTHUC,TENKH=(select TENKH 
                from KHACHHANG where KHACHHANG.MAKH=HOADON.MAKH) from HOADON`
        } else if (req.body.data == '9') {
            res.send({ done: false })
            return
        }
        let pool = await conn
        ...
        if (!isNaN(mahd)) {
            ...
            await pool.request().query(sqlBill, async (err, data) => {
                ...
                await pool.request().query(sqlDetail, async (err, data) => {
                    res.render('admin/detail-order', { title: "Chi tiết đơn hàng", detail: data.recordset, bill: hd, tentk: req.cookies.tentk, role: req.cookies.role })
                })
            })
        } else {
            //  console.log([].length == 0)
            res.render('admin/detail-order', { title: "Chi tiết đơn hàng", detail: [], bill: [], tentk: req.cookies.tentk, role: req.cookies.role })
